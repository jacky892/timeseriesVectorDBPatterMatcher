...
def is_yf_non_us(ticker):
    if '.hk' in ticker[-3:].lower():
        print('hk')
        return True
    if '.ax' in ticker[-3:].lower():
        print('ax')
        return True
    if '.t' in ticker[-3:].lower():
        print('jp')
        return True
    if '.to' in ticker[-3:].lower():
        print('toronoto')
        return True
    if '.ks' in ticker[-3:].lower():
        return True
    if '.bo' in ticker[-3:].lower():
        return True
    return False
...
class commonUtil:
    ...
    def read_quote(ticker, pred_date=None):
        if is_fx_ticker(ticker) and not is_yf_non_us(ticker):
    ...
    def download_quote(ticker):
        print(f'yf non us {ticker} %s' % is_yf_non_us(ticker))
        if ticker.lower()==ticker and not is_yf_non_us(ticker):
